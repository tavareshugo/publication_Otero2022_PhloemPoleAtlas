...
if(!("glmpca" %in% rownames(installed.packages()))){
  install.packages("glmpca", repos = "https://cloud.r-project.org")
}
...
# glmpca
message("Running GLMpca...")

metadata(sce_all)$glmpca <- glmpca(counts(sce_all[metadata(sce_all)$nucgenes, ]),
                                    L = 10,
                                    X = model.matrix(~ sce_all$Sample),# cbind(Sample = sce_all$Sample),
                                    fam = "nb")
